...
public class BasicJessBehaviour extends CyclicBehaviour {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -5601065020591856398L;
    ...
    Hashtable<String, AID> AIDCache; // holds a local cache to map agent names to AID
    ...
    public BasicJessBehaviour(Agent agent, String jessFile) {
        myAgent = agent;
        AIDCache = new Hashtable<String, AID>();
    ...
    **/
    public AID getAIDFromCache(String agentName) {
        AID result;
        result = AIDCache.get(agentName);

        if (result == null) {
            result = new AID(agentName, AID.ISLOCALNAME);
        }
    ...
    public ACLMessage JessFact2ACL(Context context, jess.ValueVector vv)
        ...
        logger.info("JessFact2ACL "+vv.toString());
        int perf = ACLMessage.getInteger(vv.get(0).stringValue(context));
        ACLMessage msg = new ACLMessage(perf);
        logger.info("******** Sender ********* " + vv.get(1).toString());
        ...
        if (vv.get(2).toString() != "nil") {
            List<AID> l = getAIDListFromCache(context, vv.get(2).listValue(context));

            for (int i = 0; i < l.size(); i++)
                msg.addReceiver(l.get(i));
        }
        ...
        if (vv.get(12).toString() != "nil") {
            List<AID> l = getAIDListFromCache(context, vv.get(12).listValue(context));

            for (int i = 0; i < l.size(); i++)
                msg.addReplyTo(l.get(i));
        }
    ...
    public class JessSend implements Userfunction {
        ...
        public Value call(ValueVector vv, Context context)
        	...
        	for (int i=0; i<vv.size(); i++) {
        		logger.info(" parameter " + i + "=" + vv.get(i).toString() +
        					" type=" + vv.get(i).type());
            }
